...
def _submit(submit_url, cookies, problem, language, files, mainclass):
    """Make a submission.
...
def show_judgement(submission_url, cfg):
    ...
    while True:
        ...
        if status_id > _RUNNING_STATUS:
            ...
            return (_STATUS_MAP.get(status_id, f'Unknown status {status_id}'), cpu_time.replace("\xa0s", '').replace("s", ""), score)

        time.sleep(0.25)


def submit(problem, files):
...
extension "{ext}"''')
    ...
    if "out of submission tokens" in plain_result:
        match = re.search(r'\b(\d+)\b', plain_result)
        if match:
    ...
    if submission_url:
        res = show_judgement(submission_url, cfg)
        if not res:
            sys.exit(1)
        print(f"{res=}")
        return res
